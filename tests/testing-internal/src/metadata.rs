...
pub enum TestResult {
    ...
    /// errors are emitted by the run for the stage, this
    /// means that the test did not fail.
    Fail,

    /// The run should not emit any errors or warnings.
    #[default]
    Pass,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum StageKind {
    /// The test should run until the parsing stage.
    Check,

    /// The test should run until the formatting stage.
    #[default]
    Fmt,
}
...
pub enum HandleWarnings {
    ...
    /// fail.
    ///
    /// If the test is expected to fail, this setting is essentially ignored.
    Disallow,
}
...
pub struct TestMetadata {
    /// The stage should the test reach before stopping.
    pub stage: StageKind,
    ...
    /// to pass specific configuration per test case.
    pub args: TestArgs,
    ...
    ///   errors, and then  base this if the run failed.
    ///
...
impl TestMetadataBuilder {
    ...
    pub fn with_stage(&mut self, stage: StageKind) -> &mut Self {
