...
use bl_testing_internal::metadata::{
...
/// case, such as whether the case should pass, or which stage it
/// should run to, etc.
#[derive(Debug, Clone)]
struct TestEntry {
...
fn create_test_entry_from_path(test_name: String, path: PathBuf) -> TestEntry {
    ...
    for warning in warnings {
        ...
        Diagnostic::new(Level::Warning, message).help(format!("occurred at file: {path:?}")).emit();
    }

    TestEntry { path, test_name, metadata }
}

fn compute_test_name_from_path(base_name: Option<&str>, path: &Path) -> String {
    let entry_snake_name = path.file_stem().unwrap().to_str().unwrap().to_case(Case::Snake);

    base_name.into_iter().chain(iter::once(entry_snake_name.as_str())).intersperse("__").collect()
}
...
fn read_tests_from_dir(
    ...
    // Special case, if the provided path is a file, and matches the
    // the test pattern, we treat it as a test case.
    if path.is_file() && test_pattern.is_match(path.file_name().unwrap().to_str().unwrap()) {
        let test_name = compute_test_name_from_path(base_name, path);

        return Ok(vec![create_test_entry_from_path(test_name, path.to_owned())]);
    }
    ...
    for entry in path.read_dir()? {
        ...
        if entry_metadata.is_dir() {
            entries.extend(read_tests_from_dir(&path, test_pattern, Some(&test_name))?);
        } else if entry_metadata.is_file()
            && test_pattern.is_match(path.file_name().unwrap().to_str().unwrap())
        {
            entries.push(create_test_entry_from_path(test_name, path));
        }
    }
...
pub fn generate_tests(input: TokenStream) -> TokenStream {
    ...
    for entry in entries {
        ...
        let test_name = format_ident!("{}_test_{}", input.test_prefix, entry.test_name);
        ...
        functions.push(quote! {
            ...
            #ignore
            fn #test_name() {
                use bl_testing_internal::metadata::{TestMetadata, StageKind, TestArgs};
